...
const fetchUserByEmail: IController = async (req, res) => {
    userService.fetchUserByEmail(String(req.query.emailId), Number( req.headers["tenant-id"]))
        .then( (user) => {
            if(user instanceof Error){
                console.log("User 2", user.message)
                apiResponse.error(
...
const deleteUser: IController = async (req, res) => {
    userService.deleteUser(Number(req.query.id), Number( req.headers["tenant-id"]))
        .then( (user) => {
            if(user instanceof Error){
                console.log("User 2", user.message)
                apiResponse.error(
