...
const fetchUserByEmail = async (emailId: string, tenant_id: number) => {
    try {
        let user = await new UserModel().fetchUserByEmail(emailId, tenant_id);
        if (user.length == 0) throw new Error("No User found");
        // console.log("customer----->",customer);
        return user[0];
    }
...
const deleteUser = async (id: number, tenant_id: number) => {
    try {
        let user = await new UserModel().deleteUser(id, tenant_id);
        if (user.length == 0) throw new Error("No User found");
        // console.log("customer----->",customer);
        return user[0];
    }
