...
export class UserModel extends BaseModel   {
    ...
    async fetchUserByEmail(emailId:string, tenant_id:number){
        return  await this._executeQuery("select * from users where email = ? AND tenant_id = ?",[emailId,tenant_id] )
        // console.log("model data--->",userResult);
    }

    async deleteUser(id:number, tenant_id:number){
        return  await this._executeQuery("update users set is_deleted = 1 where id = ? AND tenant_id = ?",[id,tenant_id] )
    }
